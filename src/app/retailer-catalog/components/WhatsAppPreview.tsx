...
export function WhatsAppPreview({ product }: WhatsAppPreviewProps) {
  ...
  const [proxiedImageUrl, setProxiedImageUrl] = React.useState('');
  ...
  React.useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const proxiedUrl = rawImageUrl ? `${origin}/api/image-proxy?url=${encodeURIComponent(rawImageUrl)}` : '';
        ...
        <div className="w-full max-w-sm mx-auto">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-2.5 shadow-sm">
              ...
              <div className="whitespace-pre-wrap text-sm text-gray-900 dark:text-gray-100">
                  {message.split('\n\n')[1]?.split('\n').map((line, index) => { // Render only the text part
                    const boldRegex = /\*(.*?)\*/g;

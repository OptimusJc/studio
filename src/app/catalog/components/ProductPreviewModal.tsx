...
export function ProductPreviewModal({ product, children, open, onOpenChange }: ProductPreviewModalProps) {
    ...
    const generateWhatsAppMessage = () => {
        ...
        if (typeof window !== 'undefined') {
          message += `\nLink: ${window.location.origin}/catalog/${product.id}`;
        }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
                <div className="p-6 flex flex-col gap-4">
                    <div className="aspect-[5/4] w-full rounded-xl overflow-hidden bg-muted relative">
                         {activeImage && <Image src={activeImage} alt={product.name} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" priority />}
                    </div>
                 ...
                 <div className="p-6 space-y-4">
                    <div>
                        <h1 className="text-3xl font-bold font-mono">{product.productCode}</h1>
                        <p className="mt-1 text-lg text-muted-foreground">{product.productTitle}</p>
                    </div>
                    
                    {product.productDescription && (
                        <div>
                            <h2 className="text-sm font-semibold tracking-wider uppercase text-muted-foreground">Description</h2>
                            <p className="mt-1 text-sm text-foreground/80">{product.productDescription}</p>
                        </div>
                    )}
                    ...
                    {specificationItems.length > 0 && (
                        <div className="space-y-3">
                            <Separator />
                            <h2 className="text-sm font-semibold tracking-wider uppercase text-muted-foreground">Specifications</h2>

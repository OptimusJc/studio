...
function CatalogContent() {
  ...
  const { data: attributesData, isLoading: isLoadingAttributes } =
    useCollection<Attribute>(attributesCollection);
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      ...
      newFilteredProducts = newFilteredProducts.filter(
        (p) =>
          (p.productTitle &&
            p.productTitle.toLowerCase().includes(lowercasedTerm)) ||
          (p.productCode &&
            p.productCode.toLowerCase().includes(lowercasedTerm)) ||
          (p.productDescription &&
            p.productDescription.toLowerCase().includes(lowercasedTerm)) ||
          (p.specifications &&
            p.specifications.toLowerCase().includes(lowercasedTerm)),
      );
    }
  ...
  const facetedSearchComponent = isLoadingAttributes ? (

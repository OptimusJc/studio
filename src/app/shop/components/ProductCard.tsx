      ...
      <div className="aspect-[5/4] relative rounded-t-lg overflow-hidden">
        ...
        <div className="absolute inset-0 bg-black/40 hidden md:flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            <div className="flex flex-col gap-2">
                <Button asChild variant="destructive" className="bg-red-600 hover:bg-red-700 w-32 rounded-lg">
                    <Link href={`/shop/${product.id}`}>See Preview</Link>
                </Button>
                <Button variant="secondary" className="w-32 rounded-lg" onClick={(e) => { e.preventDefault(); e.stopPropagation(); /* TODO: Implement similar items logic */ }}>Similar Items</Button>
            </div>
        </div>
      </div>
